...
public abstract class SymbolicValueRelation implements Comparable<SymbolicValueRelation> {
  ...
  @Override
  public int compareTo(SymbolicValueRelation o) {
    final int[] key1 = key();
    final int[] key2 = o.key();
    int answer = Integer.compare(key1[0], key2[0]);
    if (answer == 0) {
      answer = Integer.compare(key1[1], key2[1]);
      if (answer == 0) {
        answer = o.getClass().getSimpleName().compareTo(o.getClass().getSimpleName());
      }
    }
    return answer;
  }
  ...
  public Boolean impliedBy(Collection<SymbolicValueRelation> knownRelations) {
    return impliedBy(knownRelations, new TreeSet<SymbolicValueRelation>());
  }
   ...
   */
  @CheckForNull
  public Boolean implies(SymbolicValueRelation relation) {
